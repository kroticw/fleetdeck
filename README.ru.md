...
`fleetdeck.app` — тот же пульт в родном окне. Оно запускает пульт и поднимает заново, если тот остановился. Собранное из исходников, оно ещё и обновляет себя кнопкой «Обновить» в шапке.

## Установка

Из выпуска — в [последнем](https://github.com/kroticw/fleetdeck/releases/latest) лежит `fleetdeck-<версия>-macos.zip`:

1. Распакуйте и перетащите `fleetdeck.app` в «Программы» **до** первого запуска. Открытое из «Загрузок», оно запишет в настройки Claude Code временный путь.
2. Откройте из «Программ». Дальше работу принимает страница настройки.

Начиная с v0.3.0 приложение подписано Apple Developer ID и нотаризовано, поэтому macOS ничего не спрашивает. Выпуски до этого не подписаны, и macOS не откроет такой без исключения в настройках — как именно, написано в [начале работы](docs/ru/getting-started.md#установка-приложения-из-выпуска).
...
`make test` остаётся только про Go, чтобы в окружении без node всё равно проходила полная проверка Go. CI гоняет и то и другое.
