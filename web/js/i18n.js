...
const en = {
  ...
  waiting_unknown_count: "some sessions do not or cannot report whether anyone is waiting — the number beside this mark counts only the ones that do",
  ...
  update_step_download: "Downloading {version}…",
  update_step_verify: "Checking who signed it…",
  ...
  update_cannot_not_a_bundle: "This is not the fleetdeck app, so there is nothing to update. Install the app from a release.",
  ...
  update_reason_no_releases: "there is no published release to update to",
  update_reason_no_room: "there is not enough room on the disk",
  update_reason_other: "the update did not go through",
  update_reason_seal_broken: "the downloaded app has been changed since it was signed, so it was not installed",
  update_reason_seal_not_developer_id: "the downloaded app is not signed with an Apple Developer ID, so it was not installed",
  update_reason_seal_wrong_team: "the downloaded app is signed by somebody else, so it was not installed",
  update_reason_seal_no_hardened_runtime: "the downloaded app is not signed the way a release is, so it was not installed",
  update_reason_seal_no_timestamp: "the downloaded app is not signed the way a release is, so it was not installed",
  update_reason_seal_not_notarized: "Apple has not notarized the downloaded app, so it was not installed",
...
const ru = {
  ...
  update_step_download: "Скачиваю {version}…",
  update_step_verify: "Проверяю, кто это подписал…",
  ...
  update_cannot_not_a_bundle: "Это не приложение fleetdeck, обновлять нечего. Установите приложение из выпуска.",
  ...
  update_reason_no_releases: "обновляться не на что: выпусков нет",
  update_reason_no_room: "на диске не хватает места",
  update_reason_other: "обновление не прошло",
  update_reason_seal_broken: "скачанное приложение изменили после подписи, оно не установлено",
  update_reason_seal_not_developer_id: "скачанное приложение не подписано Apple Developer ID, оно не установлено",
  update_reason_seal_wrong_team: "скачанное приложение подписал кто-то другой, оно не установлено",
  update_reason_seal_no_hardened_runtime: "скачанное приложение подписано не так, как подписывают выпуск, оно не установлено",
  update_reason_seal_no_timestamp: "скачанное приложение подписано не так, как подписывают выпуск, оно не установлено",
  update_reason_seal_not_notarized: "Apple не нотаризовала скачанное приложение, оно не установлено",
