...
Durations (`poll_interval`, `silence_after`) must be a duration string such as `"30s"` or `"2s"` — a bare number is rejected with a message that says so, rather than a raw Go/YAML type-mismatch error. See `internal/config` for the full implementation and its tests.
