...
- `broken` или отсутствие ответа дольше `pageLoadWait` — повторная загрузка той же поверхности, до `pageLoadTries` раз, как у доски в T-057. После последней попытки окно убирает поверхности и показывает в доске ту же страницу «страница панели не загрузилась», что T-057 показывает для доски;
