...
**А. Свои стеклянные панели над веб-видом доски (выбран).** Одно `NSWindow`. Веб-вид доски — нижний слой на всё окно. Над ним два `NSGlassEffectView`, у каждого свой прозрачный `WKWebView`, и капсулы из системных контролов в стекле. Раскладку, отступы, радиусы и поведение задаём сами.
...
| 2 | `NSGlassEffectView`, Regular, радиус 18 | колонка оркестратора | окно, C |
| 2.1 | `WKWebView` без фона | страница поверхности `orchestrator` | окно, C |
| 3 | `NSGlassEffectView`, Regular, радиус 18 | колонка сессий | окно, C |
| 3.1 | `WKWebView` без фона | страница поверхности `sessions` | окно, C |
| 4 | ряд капсул: `NSGlassEffectContainerView` размером с ряд | объединяет стекло соседних капсул | окно, C |
| 4.1 | `NSGlassEffectView`, Regular, по одному на капсулу | вкладки, «+ карточка», тема, лимиты | окно, C |
...
**Где контейнер стекла.** `NSGlassEffectContainerView` на всё окно нет. Вид поверх всей доски забирал бы её клики: проверка попадания останавливается на самом глубоком виде под указателем, обрабатывает он щелчок или нет. Поэтому:

- у панелей контейнера нет: каждая панель — отдельный `NSGlassEffectView` прямо над доской;
- у ряда капсул контейнер есть, размером с сам ряд. Он отнимает у доски только полосу высотой 32 над её верхним отступом, где карточек нет;
- корневой вид окна и вид ряда капсул пропускают щелчок к доске там, где под указателем нет их подвида (класс `FleetdeckFrameView`).

Что теряется: стекло панели и соседней капсулы не сливается и не перетекает одно в другое (морфинг контейнера) — они остаются отдельными формами. Капсулы между собой сливаются. Как это выглядит, проверяется глазами на стенде (задача 16 плана).

**Где веб-вид поверхности.** Не внутри стекла, а над ним: вид, в который кладётся `WKWebView` поверхности, лежит в корневом виде сразу над своей панелью, с той же рамкой и скруглением 18 через слой. `NSGlassEffectView` оставляет клики у себя и в свой `contentView` их не пропускает: веб-вид, отданный стеклу содержимым, рисуется, но нажать в нём ничего нельзя. Это нашёл тест поверхностей на скрытом окне. Над стеклом прозрачный веб-вид по-прежнему показывает стекло, а стекло — доску под собой. Слои 2.1 и 3.1 таблицы — над 2 и 3, а не внутри.

**Ширина панели.** У внутреннего края развёрнутой панели — нативная полоса шириной 8 pt над стеклом и веб-видом (`FleetdeckResizeStrip`): курсор изменения размера, нажатие, перетаскивание и отпускание в координатах окна. Раму окно перекладывает на каждое движение в своём процессе; доске новые отступы уходят по отпускании: каждые новые отступы — это перекладка всей доски в её веб-виде, а стекло во время жеста показывает доску под панелью. Ширина остаётся в пределах 220 pt и 60 % окна и сохраняется в `NSUserDefaults` по отпускании. У свёрнутой панели полосы нет. Полоса страницы `col-grip` в поверхностях скрыта и остаётся только для вкладки браузера.

### 5.2 Геометрия

Числа — из макета E, в точках:

- панели отстоят от краёв окна на 8; ширина колонки оркестратора по умолчанию 368, колонки сессий 348; свёрнутая колонка — 48, у оркестратора и у сессий;
- ряд капсул: сверху 10, высота 32, между панелями: 10 от левой и 12 от правой. Центр ряда на 26 от верха окна — на одной линии с кнопками окна и верхними строками обеих панелей. До v0.10.2 было 12, и центр ряда стоял на 28;
- у свёрнутой колонки оркестратора кнопки окна кончаются дальше её полосы (кнопка увеличения — на 79), и вне полного экрана ряд капсул начинается в 10 от кнопки увеличения, а не от полосы; панель сессий при нехватке места сужается с учётом этого. В полном экране кнопок на этой линии нет, и ряд снова идёт в 10 от полосы. Доска по-прежнему начинается в 18 от полосы. До правки в v0.10.2 вкладки Board/Docs лежали под кнопками;
- доска получает отступы: слева — правый край левой панели плюс 18, сверху 64, справа 0 — последняя колонка уходит под панель сессий, и стекло показывает доску под собой. Отступы окно сообщает странице доски (раздел 6.2), страница применяет их CSS-переменными `--host-inset-top`, `--host-inset-left`, `--host-inset-right`;
- то, что открывается над доской — лист K1/S1, доки Д1, плашка сборки, — держится в стороне от панели сессий: у него свой правый отступ, ширина панели сессий плюс её поле (356 при ширине 348, 56 при свёрнутой). Окно шлёт его в том же сообщении, страница применяет переменной `--host-inset-content-right`;
- кнопки окна (светофоры) AppKit рисует поверх содержимого. Центр кнопки закрытия стоит на (26, 26) от левого верхнего угла окна, то есть концентрично скруглению 18 острова оркестратора с полем 8, как в плавающей боковой панели Finder. Верхняя строка колонки оркестратора оставляет под кнопки место слева, а её ряд стоит на той же линии: окно сообщает поверхности, где кончается кнопка увеличения и на какой высоте центр кнопок (раздел 6.2, сообщение `titlebar`).

Окно включает `NSWindowStyleMaskFullSizeContentView` и `titlebarAppearsTransparent`: содержимое идёт под заголовок, заголовок не рисуется, светофоры остаются. Кнопки ставит пустой `NSToolbar` стиля `unified`: без панели инструментов AppKit ставит центр кнопок на (16, 16), на край острова, как было в v0.10.1. У панели инструментов нет элементов, под прозрачным заголовком она ничего не рисует. Замер на скрытом окне macOS 27: рамка окна, вид содержимого и `contentMinSize` от неё не меняются, щелчки по полосе перетаскивания, капсулам и шапке оркестратора доходят до них, а не до заголовка.
...
- Проверка Origin (`guard.go`, `isLoopbackOrigin`) смотрит только схему `http`/`https` и loopback-хост; порт она не проверяет вовсе. Поверхности открывают настоящие адреса панели — их запросы и сокеты проходят её так же, как страница сегодня. Терминал, как и сейчас, берёт токен `/api/terminal-token` на каждый сокет; токен общий для процесса панели.
...
| `insets` | доска | `top`, `left`, `right` (прокручиваемая доска) и `contentRight` (лист, доки, плашка) в точках |
...
- Три снимочных сокета `/ws` вместо одного. Сервер держит одну горутину и тикер на соединение (`ws.go`); ограничений на число клиентов нет. Но на каждом тике каждый сокет заново фильтрует снимок по флоту (`push`, `ws.go:74` → `fleetView`, `api.go:114` → `state.ForFleet`), поэтому эта работа и объём JSON по петле утраиваются. Сколько это стоит, мерится на стенде с порогом (раздел 10.3).
- Терминал оркестратора живёт в поверхности `orchestrator`, терминал открытой сессии — в листе S1 доски. Два подключения к одной сессии на macOS работают одновременно (`daemon-control-socket.md`), но размер терминала общий: последний подключившийся задаёт размер PTY всем остальным (`live-terminal.md`, §2).
...
| перезагрузка после смены сборки | `sessionStorage`: откуда уходим, флот, открытая сессия, число попыток | `sessionStorage` у каждого вида свой. Сборку проверяет только доска: её `fleetdeckReload` перезагружает все три вида, а число попыток лежит в её хранилище. Поверхности оркестратора и сессий сборку не проверяют, плашку не рисуют, сами не перезагружаются и в `sessionStorage` о перезагрузке не пишут. Перезагрузка без записей в хранилище сняла бы потолок попыток |
...
### 6.6 Готовность поверхностей

Белое окно T-057 случилось, когда окно сочло страницу показанной в момент навигации, а не в момент загрузки. Две новые поверхности повторили бы его вдвое, поэтому они подчиняются тому же подтверждению загрузки, что T-057 вводит для доски (ветка `fix/update-staged-bundle-relaunch`, на момент спеки не смержена; имена сверяются в задаче 0 плана):

- в каждый веб-вид поверхности подмешивается тот же `pageLoadScript`, что и в доску. По событию `load` он сообщает через `fleetdeckPageLoaded` одно из `panel`, `broken`, `leaving`. Состояние `panel` значит: стили пришли, и `main.js` поставил метку `data-fleetdeck-page="running"`;
- ES-модули выполняются до события `load`, а `main.js` подключает приём сообщений окна синхронно при выполнении. Поэтому `panel` от поверхности означает и «`main.js` смонтирован и слушает» — отдельной привязки готовности нет;
- **до `panel` окно не шлёт поверхности `receive()` и не делает в неё `evaluateJavaScript:`.** После `panel` окно отдаёт ей текущее состояние целиком (тема, режим стекла, свёрнутость, полный экран), а не накопленную очередь;
- `broken`, провал навигации по слову WebKit или отсутствие ответа дольше ожидания — повторная загрузка той же поверхности, до `pageLoadTries` раз, по тем же правилам, что у доски после T-059: у каждой поверхности свой `pageWatch` (`navscreen.go`), делегат навигации поверхности сообщает события WebKit; ожидание `navSilentWait` до коммита навигации и `pageLoadingWait` от коммита, пауза `navFailedRetryPause` перед повтором после провала, `NSURLErrorCancelled` (−999) и `WebKitErrorFrameLoadInterrupted` (102) — не провал, слово страницы до коммита не принимается; процесс WebContent поверхности, завершившийся под показанной страницей, — одна перезагрузка, повторная потеря в пределах 30 с — окно убирает поверхности и показывает в доске страницу «Страница панели падает». После последней попытки окно убирает поверхности и показывает в доске ту же страницу «страница панели не загрузилась», что T-057 показывает для доски;
- `leaving` снимает готовность: до следующего `panel` поверхность снова ничего не получает.

### 6.7 Куда уходят переходы

Веб-виды, которые окно создаёт само, всегда остаются на своей странице панели. Навигацию решает делегат (`WKNavigationDelegate`, `decidePolicyForNavigationAction:`):

| Переход | Решение |
| --- | --- |
| загрузка своей страницы: адрес панели с тем же `fleet`, перезагрузка | разрешить |
| тот же origin, другая страница (`/setup.html` из ссылки «мастер…», `/?fleet=<другой>`, `/` без флота) | отменить; доска переходит на этот адрес, поверхности убираются и вернутся после её отчёта `panel` |
| другой origin (`http`, `https`) | отменить; открыть в браузере системы через `NSWorkspace openURL:` |
| остальные схемы (`file:`, `data:`, `javascript:`) | отменить |
| окно «в новой вкладке» (`targetFrame == nil`, `target="_blank"`) | по тем же правилам; `WKUIDelegate createWebViewWithConfiguration:` возвращает `nil` |

Через мост в поверхностях доступны все привязки реестра, включая `fleetdeckUpdate`, `fleetdeckUpdateKnown` и `fleetdeckReload`: кнопка обновления живёт в поверхности оркестратора.

### 6.8 Жизненный цикл видов поверхностей

Поверхности создаются по отчёту доски и убираются при смене флота, при своей странице окна и при отказе загрузки — за время жизни окна это происходит многократно. Код не под ARC, поэтому освобождение описано явно:

- **создание:** окно выделяет `WKWebViewConfiguration` (из неё берётся `WKUserContentController`), `WKWebView`, экземпляр обработчика сообщений и делегат навигации;
- **уничтожение, по порядку:** `stopLoading`; делегат навигации и UI-делегат — `nil`; `removeScriptMessageHandlerForName:@"fleetdeck"` — это рвёт удержание обработчика контроллером содержимого; `removeAllUserScripts`; `removeFromSuperview`; `release` веб-вида, конфигурации, обработчика и делегата — ровно тех, что окно выделило само;
- **классы** обработчика и делегата регистрируются в рантайме один раз за процесс и не освобождаются;
- **проверка:** тест создаёт и уничтожает поверхности многократно и сверяет, что живых экземпляров обработчика ноль, а в контейнерах панелей нет подвидов (задача 10 плана).
...
- `NSGlassEffectContainerView` у ряда капсул не создаётся; капсулы — прямые подвиды вида ряда;
...
- что в macOS 26 «Увеличить контраст» само включает и фиксирует «Уменьшить прозрачность», первичным источником не подтверждено — **проверить на стенде** (раздел 10.1). Подтвердится — отдельной ветки для контраста нет; нет — окно читает и `accessibilityDisplayShouldIncreaseContrast` и при включённом контрасте выбирает `opaque`.
...
- **Двойной старт панели при передаче.** Во время `Takeover` панель стартует дважды: из промежуточного бандла, затем из канонического после обмена. После T-057 новое окно не просит страницу панели, пока передача не завершена (`takingOver`, `handed` в `screen`), и держит свою страницу. Правила контроллера для этой последовательности:
  - своя страница окна — поверхностей нет или они убираются;
  - первый `panel` доски с флотом — поверхности создаются один раз;
  - повторный `panel` с тем же флотом (страница доски перезагрузилась, панель ответила снова) — поверхности заново не создаются; доске заново уходят отступы, тема и режим стекла, потому что это новая страница;
  - `panel` с другим флотом — поверхности пересоздаются на новый флот;
  - перезапуск панели при показанной странице ничего не создаёт и не убирает: страницы переподключаются сами, как сегодня.

  Последовательность закреплена тестом контроллера (задача 11 плана).
- **Перезагрузка после смены сборки.** Сборку проверяет только доска: `buildcheck.js` в ней вызывает `fleetdeckReload`, и окно перезагружает все три веб-вида одной командой — доску навигацией на адрес панели, как сейчас (`reloadBinding`), поверхности на свои адреса. Поверхности оркестратора и сессий `buildcheck.js` не запускают (раздел 6.4).
...
| плашка устаревшей страницы (`#build-banner`) | над доской под капсулами, в стороне от панели сессий | `board` |
...
### 10.1 Стенд на 26.6.2 у оператора

Стенд части Б — по правилам, которые ввёл T-057 после инцидента 2026-09-14, когда окно стенда запускало свои панели без изоляции. Одних `HOME` и порта мало:

- окно передаёт стендовую изоляцию (`FLEETDECK_STAND_SOCKET`) **каждой** панели, которую запускает, включая перезапуск после обмена; без неё панель на стенде не запускается;
- в логе каждой панели стенда есть строка `daemon discovery disabled`; скрипт стенда проверяет её до того, как окно покажет что-либо;
- бандл стенда — с `BUNDLE_ID=dev.fleetdeck.stand`, регистрации LaunchServices снимаются после стенда;
- свой `HOME`, свой порт, не 7777.

Проверяется глазами оператора, со снимками:

- обе темы и `auto` при светлой и тёмной системе;
- «Уменьшить прозрачность» и «Увеличить контраст» — включает оператор; для контраста записывается, включает ли он сам «Уменьшить прозрачность» (раздел 7.2);
- полный экран;
- K1, S1, Д1, свёрнутая колонка сессий, перетаскивание ширины;
- печать в терминал листа S1 сразу после открытия из колонки сессий;
- ссылка «мастер…» из колонки оркестратора открывается в доске, внешняя ссылка — в браузере системы (раздел 6.7);
- смена флота из строки бренда в поверхности оркестратора и из колонки сессий;
- область под стеклом обновляется: два снимка с интервалом и сравнение, как в спайке (раздел 3).
...
**Обновление кнопкой — обязательная проверка**, это пункт «нельзя сломать». На стенде по процедуре T-057: окно предыдущей сборки обновляется до сборки ветки части Б, передача доходит до `done`, новое окно показывает доску с панелями, поверхности создаются один раз (раздел 8). Без этой проверки часть Б не сдаётся.

### 10.2 macOS ниже 26

Решено: **запуск окна в CI на раннере `macos-15`**. Эту проверку уже делает T-058 (PR #163): задачи `oldest-macos-app` и `window-on-oldest-macos` в `.github/workflows/ci.yaml` и скрипт `scripts/ci-window-stand.sh`. Они собирают приложение как выпуск, запускают окно на стенде (свой `HOME`, порт, `FLEETDECK_STAND_SOCKET`), ждут в логе окна подтверждения страницы `panel` и снимают окно. Отдельной задачи для окна со стеклом нет: после мержа T-058 её проверка расширяется — ждать подтверждения `panel` и от поверхностей `orchestrator` и `sessions`, а не только от доски, и снимать окно с рамой (задача 17 плана). На `macos-15` стекла нет, поэтому снимок показывает раму на `NSVisualEffectView` (раздел 7.1).

Tart с macOS 15 — только если оператор сам захочет посмотреть на окно глазами; план его не содержит.

### 10.3 Замеры и пороги

Памяти на машине оператора мало, а новое устройство добавляет два процесса WebContent и утраивает работу сокетов. Поэтому замеры — числами и с порогами, на одном стенде и одном флоте: сначала окно до части Б (база), затем окно части Б, по 60 с с открытым окном:

| Что | Как мерить | Порог |
| --- | --- | --- |
| память процессов WebContent окна | `footprint` по PID процессов `com.apple.WebKit.WebContent`, появившихся после запуска окна: сумма и каждый | сумма не больше базы + 250 МБ; каждый процесс не больше 200 МБ |
| CPU панели | средний `%CPU` процесса `fleetdeck` стенда за 60 с (`ps -o %cpu=` раз в секунду) | не больше базы × 1,5 и не больше 3 % одного ядра |
| объём `/ws` | байты, отданные панелью стенда за 60 с (`nettop -p <pid>`) | не больше базы × 3,2 |

Порог превышен — часть Б не сдаётся, и окно не оптимизируется наугад: причина приносится оркестратору. Очевидный путь для CPU и `/ws` — считать `fleetView` один раз на тик на флот, а не на сокет; но это правка сервера и отдельное решение.
...
| регрессия стекла 26.2 вернётся в другой версии | сравнение области под стеклом на стенде (раздел 10.1) |
...
| три снимочных сокета и два терминала вместо одного | раздел 6.3, замеры с порогами — 10.3 |
| новые веб-виды повторяют белое окно T-057 | готовность поверхностей, раздел 6.6 |
| поверхность уходит с адреса панели | делегат навигации, раздел 6.7 |
| утечка веб-видов при смене флота | жизненный цикл, раздел 6.8 |
