...
Решено: **дымовой запуск в CI на раннере `macos-15`**, отдельной задачей плана, в стык с T-058. Задача собирает бандл окна, запускает его со стендовой изоляцией (`HOME`, порт, `FLEETDECK_STAND_SOCKET`), снимает окно в артефакт и проверяет, что процесс жив через заданное время. Это ловит «не запускается» и «падает при старте», которые сегодня есть в выпуске.
