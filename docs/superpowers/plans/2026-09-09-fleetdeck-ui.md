...
> **Поправка, внесённая при исполнении.** Код встраивания ниже не компилируется: `//go:embed all:../../web` даёт `invalid pattern syntax`, потому что шаблон `go:embed` не может выйти за каталог своего пакета. Значит `internal/server/static.go` физически не способен встроить `web/`, лежащий в корне репозитория.
>
> Встраивание живёт в `web/embed.go` с `package web`, где шаблоны относительны и легальны; `internal/server` импортирует готовую файловую систему. Причина записана комментарием в самом пакете: следующий, кто захочет перенести каталог, должен узнать это из кода, а не из упавшей сборки.
>
> По той же причине в шаблонах нельзя перечислять пути, которых ещё нет: несуществующий путь в `go:embed` — тоже ошибка компиляции, а не пустое встраивание.
