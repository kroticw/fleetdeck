...
- `board` — an empty board made from the board template this repository carries (`plugin/templates/board`): a `cards` directory with no cards in it, `README.md`, `archive/AGENTS-ARCHIVE.md`, and the board's scripts in `scripts/` with their tests: the card validator, card creation with a number, lookup of a card by its number, and number backfill for older cards. It is a git repository of its own, on `master`, with no commit yet: a signed commit would ask for a passphrase, and the panel has nobody to ask.
...
The wizard can be run again from the orchestrator column's head (**wizard…**), which opens the same page at this step. There it marks the current orchestrator and says what happens to it when another is chosen: it leaves the column, and the line it was sent stays in its history. The column's dropdown, beside it, only moves the pin and sends nothing — so a session pinned through it has no working order on disk, and the column says so, naming the file, until an appointment writes one. It says the same about a working order that was deleted after an appointment wrote it: the file is looked for on every cycle, not once while the wizard runs, because a brief written and later removed is indistinguishable on disk from one that never existed. And it says so, in other words, when a file fleetdeck did not write stands where the working order belongs: the pinned session would read that file as its working order, and the wizard will not replace it, so the row asks for the file to be moved aside before the wizard is run. Whether the file is fleetdeck's is read from its first line on every cycle, the same line the wizard checks before it writes.
...
The fleetdeck app starts the panel itself, and the panel lives exactly as long as the app. When the app opens, it looks at the panel's address. A panel that already answers there and was started from a terminal is shown as it is, and so is the panel of another fleetdeck window that is still open. If such a panel is not the app's own build, the window says so in a strip along the bottom of the page: the panel's build, the app's, and where the panel was started from; the build in the header is then marked as the panel's. A panel started from a terminal can be replaced with the app's own from there. A panel kept by the launch agent of an earlier `init` is not replaced — the agent would only start it again — and the strip shows the two commands that remove the agent (the same ones as below). A panel whose window is gone — one left behind by an older app that did not stop its panel, or by an app that ended some way its panel did not notice — is stopped, and the window says so while it starts its own. When nothing answers, the app starts the panel it carries, sends both of the panel's output streams to `~/Library/Logs/fleetdeck.log`, and opens it as soon as it answers.
...
A browser pointed at that address gets the start page, and `?fleet=<name>` on it gets that fleet's panel. The routes the page uses are callable directly too — `GET /api/snapshot` for the state of the whole fleet, `GET /ws` for a live stream of it, `GET /api/sessions/{id}/pty` for a live two-way terminal on one session (a WebSocket whose geometry is given as `?cols=&rows=`), `GET /api/docs` and `GET /api/docs/content` for the directories `docs.paths` names, and the routes that resume a session and move a card. Only pages the panel served itself may call them: a request carrying any other `Origin` is refused with 403, and a request body that is not `application/json` with 415. The terminal socket carries no request body, so it has a check of its own in place of the second one: its first message must be the panel's terminal token, which `GET /api/terminal-token` hands to the panel's own page and, carrying no CORS headers, to no page on any other origin. The token is new with every start of the panel, and the page reads it again for every terminal it opens.

An image goes to a session the way it does in Claude Code itself: copy it, click into the session's live terminal and press Ctrl+V — Ctrl, not Cmd. Claude Code reads the image from this Mac's clipboard and puts `[Image #N]` into its prompt; the panel stores nothing of its own.
...
In the fleetdeck app on macOS 26 the board fills the window, and the orchestrator and sessions columns are glass panels over it; the tabs, the new card button, the theme and the limits sit in glass capsules between them. A card, a session or a document opens as a sheet over the board. A panel is made wider or narrower by dragging its inner edge, where the pointer turns into the resize cursor; it stays between 220 points and 60% of the window. In a narrow window the limits become one capsule of text and then the theme becomes an icon, the panels narrow on screen but not below 220 points while keeping the width you set, and the window does not get narrower than room for all of this. The capsules follow the system's light or dark mode, even when the board's theme is set otherwise. The fold button folds the panel itself. The app, not the page, remembers both the width and the fold across launches. A browser tab on the same panel keeps the three columns described above.

The type in a live terminal — the orchestrator column's and the session panel's — is resized from the keyboard while the terminal has the focus: Cmd+= (or Cmd+Shift+=) makes it bigger, Cmd+- smaller, Cmd+0 brings back the 12 px it starts at. It goes from 9 to 24 px, and each of the two places remembers its own size across a reload. A bigger type in the same pane leaves fewer columns, and the columns are the session's own: every change reshapes the session for everyone attached to it, a `claude attach` in Terminal.app included, just as dragging the column's edge does. So the new size is shown over the terminal for a moment — the type's and the session's, as in "14 px · session 69 × 25" — and at either end of the range the same note says that it is the limit.

The same three steps are buttons too: A−, the current size, and A+. In the orchestrator column they sit in the strip at its top, beside the fold button; in the session panel, in its header beside the session's name. The size between them is the button that brings back 12 px. Each does exactly what its key does, and the button that cannot do anything at the current size is greyed out: A− at 9 px, A+ at 24 px, the size at 12 px. Pressing one leaves the keyboard focus in the terminal. When the session panel is too narrow to hold them beside the session's name and the close button, its header leaves them out; the keys still work there.

A session opened from the session list shows its live terminal, with nothing to switch to: you read the session and write to it there, the same way as in `claude attach`. What has scrolled off the screen, or happened before the panel was opened, is behind the transcript key under the terminal: it presses Ctrl+O, Claude Code's own detailed transcript, and pressing it again comes back. Above the terminal is the list of cards the session has worked on, oldest first, finished and archived ones included — the one thing about a session its own screen does not show. A card still on the board opens from there; an archived one is drawn dashed and does not open. A session that has not taken a card says so rather than showing an empty row.
