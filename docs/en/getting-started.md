...
In the fleetdeck app on macOS 26 the board fills the window, and the orchestrator and sessions columns are glass panels over it; the tabs, the new card button, the theme and the limits sit in glass capsules between them. A card, a session or a document opens as a sheet over the board. A panel is made wider or narrower by dragging its inner edge, where the pointer turns into the resize cursor; it stays between 220 points and 60% of the window. The fold button folds the panel itself. The app, not the page, remembers both the width and the fold across launches. A browser tab on the same panel keeps the three columns described above.
