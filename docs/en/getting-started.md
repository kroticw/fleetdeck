...
In the fleetdeck app on macOS 26 the board fills the window, and the orchestrator and sessions columns are glass panels over it; the tabs, the new card button, the theme and the limits sit in glass capsules between them. A card, a session or a document opens as a sheet over the board. The fold button folds the panel itself, and the app, not the page, remembers it across launches. A browser tab on the same panel keeps the three columns described above.
