...
- **Decided:** the terminal takes its size back. When the stream places the cursor at an absolute column past the terminal's last column, it sends its own size again after 150 ms, and then at most once a second while that keeps happening (`web/js/liveterminal.js`, `positionWatcher` and `takeSizeBack`). Nothing else counts: a column equal to the last, a row past the bottom, a relative move, wide characters at the edge, the alternate screen, or a column past 500 (an application finding its corner). A terminal that cannot be measured takes nothing and takes the size back once it can. Two terminals of different widths settle on the narrower one: a narrower screen fits in a wider terminal, so the wider one never takes the size back. `web/tests/terminalwidth.test.js` replays the daemon's way of drawing through xterm's own core and compares the result with a fresh attach.
