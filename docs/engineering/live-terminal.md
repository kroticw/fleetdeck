...
- **Decided:** the terminal takes its size back. When the stream places the cursor at an absolute column past the terminal's last column, it sends its own size again after 150 ms, and then at most once a second while that keeps happening (`web/js/liveterminal.js`, `followSize` and `takeSizeBack`). The positions are watched by hooks in xterm's parser as it reads the stream, which also reads a sequence cut between two pieces as one. xterm reads a moment after the stream arrives, so a screen that arrived before the size was last sent does not count: the repaint for that size is already behind it. Nothing else counts: a column equal to the last, a row past the bottom, a relative move, wide characters at the edge, or a column past 500 (an application finding its corner). The alternate screen is not tracked, so switching to it is no reason by itself. A terminal that cannot be measured takes nothing back and takes the size back once it can; a pane still moving is taken back from once it settles. Two terminals of different widths settle on the narrower one: a narrower screen fits in a wider terminal, so the wider one never takes the size back. An attacher that stays longer than the wait is taken back from while it is attached, and again when it leaves if the daemon then gives the session a wider attacher's size: what bounds it is once a second, not once per visit.
- **Measured: taller at the same width breaks the bottom.** The daemon reaches each next line with a cursor down (`ESC[1B`), and xterm stops the cursor at the last row, so every row past it is drawn over the last one. On the same disposable session, a 76 × 60 attacher beside a 76 × 40 terminal piled 20 rows onto the last one and hid the input line; the terminal's own resize repaired it. **Decided:** a cursor moved down past the last row counts too. Only xterm knows where the cursor is when the move arrives, so this is a hook in xterm's parser (`followSize`), not a look at the bytes. A move of more than 200 rows is an application finding the bottom and does not count, and neither does an absolute row past the bottom.
- **The trade-off, named:** while this terminal is open, every wider viewer of the session gets its width 150 ms after it attaches or resizes. That includes the operator's own `claude attach` in Terminal.app, at any width. A narrower screen draws correctly in a wider terminal, so that viewer sees a narrower screen, not a broken one. `web/tests/terminalwidth.test.js` replays the daemon's way of drawing through xterm's own core and compares the result with a fresh attach.
