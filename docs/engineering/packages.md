...
- **`internal/transcript`** — reads session transcripts from the tail: locating a session's `.jsonl` file and an estimate of context-window occupancy for when the statusline reporter is not installed. It is the source for what a session *did*; it never reports what a session is doing now.
