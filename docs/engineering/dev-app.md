...
At the next start all of that is replaced from the operator's config, with one exception: **a fleet made from a dev panel is kept**, since the panel asks to be restarted to work in it. A fleet of the previous copy whose name no fleet of the operator's config has is carried into the new copy, whole; a fleet of the same name in the operator's config is the one that stays. On a port with no copy yet, the fleets are taken from `~/.config/fleetdeck/dev/config.yaml`, the one copy all dev apps shared before copies were kept per port; that file is read and left as it is.
