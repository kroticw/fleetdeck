...
It passes only when the new window reports `alive`, `panel`, `swapped` and `done` in that order within the deadline; the panel at the URL reports this branch's commit and an executable inside `/Applications/fleetdeck.app`; and `/Applications/.fleetdeck-update/fleetdeck.app` answers `v0.10.0`. Once the program has exited, as the old window does after its update returns, a step of its own checks that the new window removed that bundle within 30 s and that LaunchServices (`lsregister -dump`) knows `/Applications/fleetdeck.app` exactly once and no path under `/Applications/.fleetdeck-update`. The old version is pinned to v0.10.0.
