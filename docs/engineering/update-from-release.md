...
It passes only when the new window reports `alive`, `panel`, `swapped` and `done` in that order within the deadline; the panel at the URL reports this branch's commit and an executable inside `/Applications/fleetdeck.app`; and `/Applications/.fleetdeck-update/fleetdeck.app` answers `v0.10.0`. LaunchServices (`lsregister -dump`) is then required to know `/Applications/fleetdeck.app` exactly once and no path under `/Applications/.fleetdeck-update`, twice, within 60 s each: first while the program — the old window — is still running and the new window has said the handover is done, so the bundle swapped out is still on disk; and again once the program has exited, from a step of its own, after the new window has removed that bundle within 30 s. The first read is a harder order than the operator's, whose v0.10.0 window quits as soon as it reads done, so their new window may well remove that bundle before its window is made. The dumps are kept in the job's artifact. The old version is pinned to v0.10.0.
