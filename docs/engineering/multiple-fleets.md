...
- **A fleet added while the panel runs.** It is served only after a restart. The start page can now add one (`POST /api/fleets`, #125) and says so before its button and again after it; `init --fleet` says the same. There is still no hot reload of the fleet list, and adding one would mean rebuilding everything the running panel derived from the configuration it read at start: the collector, the per-fleet accessor, the first fleet's capabilities on `Deps`, and one board watcher per fleet. `cmd/fleetdeck/startfleet_test.go` pins that the running panel really does not serve the new fleet, so neither half of that sentence can drift without a test failing.
