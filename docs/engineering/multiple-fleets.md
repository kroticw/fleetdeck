...
**Amended by the start page (#125): the bare root is no longer the first fleet's panel.** `GET /` with no `fleet` key at all serves the start page (`web/start.html`), where a fleet is chosen or a new one made; the panel is `/?fleet=<name>`. The rule above still holds everywhere else — every route that serves a fleet reads the parameter through `Deps.forFleet`, and an absent one is still the first fleet — and it holds at the root too for `/?fleet=`, the parameter present and empty, which is what `withFleet` builds for a panel that has only ever had one fleet and what the wizard now leaves for. What changed is one case: an address carrying no fleet key opens a list instead of a panel. An old bookmark of `/` therefore costs one click.

**Measured by review, not by a test: the window's own reload had to be carried back.** The window reloads by navigating to its fixed `-url`, so a panel replacing itself under the operator landed on the start page — and lost not only the fleet but the session panel that was open, because the start page never runs `takeOpenSession` and the first fleet it is sent to clears the key (`web/js/fleet.js` `switchFleet`). That broke, silently, the guarantee `web/js/buildcheck.js` states in so many words: a window reloading by itself must not close the panel under someone for no reason they can see. So `reloadNow` writes the fleet down beside the build it is reloading from, and the start page carries the page straight back into it, keeping the open session — it is returning, not leaving. Taken once, so only that one load hops; every other visit stays on the list. A fleet no longer in the configuration is not hopped into: its snapshot would answer 404.
...
- **A fleet added while the panel runs.** A fleet made from the start page (`POST /api/fleets`) is served at once. After init's steps the panel reads the fleet back from the file and adds it to what it runs (`liveFleets` in `cmd/fleetdeck/main.go`): the collector's fleets, which every cycle and the per-fleet accessor read on each use, and a watch of its board; the panel refreshes before the request answers. The collector alone decides whether the fleet is new, so a fleet made twice is watched once. A fleet the running panel would refuse — its name or its board taken by a fleet it serves, which happens when the file was edited by hand — is a failed `panel` step, not a made fleet. Nothing else is reloaded: the first fleet's capabilities on `Deps` never change, and the file is not watched, so `init --fleet` from a terminal and any edit by hand take effect after a restart, as `init` says. `cmd/fleetdeck/startfleet_test.go` pins both: a fleet made from the start page is served at once, and a fleet `init --fleet` writes past the running panel is not. The `panel` step is the last one, and the start page answers a refusal there with its own sentence, not with "the fleet was not made".
