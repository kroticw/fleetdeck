...
- Until the page has said so, the window asks for it again when a panel answers again and each time `pageLoadWait` goes by, `pageLoadTries` times in all. After that it shows a page saying the panel's page did not load, with a button to try again.
