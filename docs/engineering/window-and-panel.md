...
- The window sets its own navigation delegate (`nav_darwin.c`) and logs what WebKit says of each navigation: started, committed, finished, failed with its error, and the web content process it ran in. It logs each page of its own it puts up, by its heading.
- Until the page has said so, the window asks for it again when a panel answers again, when WebKit says the navigation failed or its web content process went away, and when time goes by: `navSilentWait` for a navigation WebKit has said nothing of or has only begun, `pageLoadingWait` for one whose document has begun (WebKit says it committed, or the script says `loading` as the document starts). A navigation that failed because another took its place (`NSURLErrorCancelled`, or WebKit's frame load interrupted) is not the page failing. Asking again cuts a load off and starts it over, so a navigation is not asked for again while the window's web content process may still be starting. The window asks for the address the page last said it was at, not its own URL: its own URL is the start page, and a page that left for a fleet (`?fleet=`) would lose it. A page that left and has not yet said where the next one began is not asked for at all — the window does not know where it went — and is only reported if nothing begins within `pageLoadingWait`. After `pageLoadTries` in a row the window shows a page saying the panel's page did not load, with a button to try again.
