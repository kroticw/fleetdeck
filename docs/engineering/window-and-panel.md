...
8. It exchanges the staged and installed bundles in one system call: `renamex_np` with `RENAME_SWAP` on macOS, `renameat2` with `RENAME_EXCHANGE` on Linux. Then it reports `swapped`, and has LaunchServices forget the staged path and take the canonical one (see "Where the next launch goes").
...
On 2026-09-14 an update from v0.9.0 to v0.9.1 went through, and the next time the operator opened the app it was v0.9.0, running from `/Applications/.fleetdeck-update/fleetdeck.app`. The new window had started from the staged path, and a Cocoa app checks in with LaunchServices when it starts: from then on the database held two bundles under `dev.fleetdeck.window`. After the swap, the staged path held the old version.
...
Not measured: whether macOS checks a window running from the staged path in again when it is later activated. That is why the staged bundle does not survive the update (step 10): nothing goes back to it — a failed update fails before the swap and leaves the canonical bundle as it was — and left in place, it is the old version under the app's own identifier. It is removed only after the old window's update has let go of the lock, because until then the old window is running out of it.
...
- Until the page has said so, the window asks for it again when a panel answers again, and when time goes by: `pageLoadWait` for a navigation that never reached its document, `pageLoadingWait` for one whose document has begun (the script says `loading` as the document starts). Asking again cuts a load off and starts it over, so a page that is loading is not asked for again at the short wait. The window asks for the address the page last said it was at, not its own URL: its own URL is the start page, and a page that left for a fleet (`?fleet=`) would lose it. A page that left and has not yet said where the next one began is not asked for at all — the window does not know where it went — and is only reported if nothing begins within `pageLoadingWait`. After `pageLoadTries` in a row the window shows a page saying the panel's page did not load, with a button to try again.
...
`pageLoadWait` was measured on 2026-09-14 on the operator's machine, with the fleet running, on a stand with its own HOME, port and fleet and a `-stand-socket` panel. Twenty windows were started one after another, each a new process with a cold web view, and timed from the navigation to the page saying it had loaded. The board, in ms: 177, 94, 94, 89, 89, 93, 90, 94, 94, 92 (the first slower, as a web view's first start is). The start page: 62, 63, 55, 61, 65, 56, 64, 61, 60, 62. The worst, 177 ms, three times over is 531 ms: `pageLoadWait`, the wait for a navigation to reach its document. A load straight after login, with nothing in the disk cache, and a board of hundreds of cards were not measured; a slower page is asked for again, not given up on.

`pageLoadingWait`, the wait for a document that has begun, came from the update stand. Its first run, before the window could tell a document begun from one never reached, asked for the start page three times after `done`: the third load finished 280 ms after it was asked for, and the first two were cut off by the next ask. Three times 280 ms is 840 ms. Nothing measured since came near it — five freshly built binaries on a warm HOME reached the document in 53–74 ms and finished in 67–88 ms, five windows in HOMEs no web view had run in took 56–70 and 69–84 ms, and the second stand run 13 and 29 ms — and what made that first run slow was not found.
...
- **Decided: a surface is sent nothing before its page says it loaded.** Each surface gets the same `pageLoadScript` as the board. Until it reports `panel`, the window neither sends it messages nor evaluates anything in it. A load not heard from within `pageLoadWait` (or `pageLoadingWait` once the document has begun) counts as broken: reloaded up to `pageLoadTries` times, then the frame goes and the board shows the failed-load page. The screen's own waiting and retrying for the board is not changed by the frame.
