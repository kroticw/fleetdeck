...
- **Measured: v0.9.1 does not open from Finder below macOS 26, and dyld is not why.** `xcrun vtool -show-build` on v0.9.1: `fleetdeck-window` names `minos 26.0` in both slices, the panel and `fleetdeck-status` `minos 13.0`, and `Info.plist` promises `LSMinimumSystemVersion` `11.0`. Nothing in the build named a macOS, so clang built the window for the macOS of the runner, `macos-latest`, which is macOS 26. On a GitHub `macos-15` runner (macOS 15.7.9, 24G830), CI's `window-on-oldest-macos` found:
  - Started directly, v0.9.1's window loaded, started its panel and showed the fleet page (a screenshot is in the job's artifact). dyld did not refuse it.
  - Opened through LaunchServices, the way Finder opens it, the same bundle was refused: `_LSOpenURLsWithCompletionHandler() failed with error -10825`. `LSConstants.h` names that `kLSIncompatibleSystemVersionErr`, "The app cannot run on the current OS version". Its plist promises 11.0, so the refusal comes from what the executable names.
  - A copy of this build with `LSMinimumSystemVersion` set to 26.0 was refused with the same error.
  - This build, `minos 13.0` in every slice, reached a page that reported itself loaded with its styles and scripts (`the panel's page says "panel"`) both opened through LaunchServices and started directly, and its panel logged `daemon discovery disabled`.
  - v0.9.1's window logs no page state and knows nothing of `FLEETDECK_STAND_SOCKET`, so its panel on the runner looked for a fleet daemon as usual. There was none to find. Do not run v0.9.1's window as a stand on a machine with a fleet.
- **Read, not run: v0.9.1's window cannot load below macOS 15.4, even started directly.** It imports `std::bad_function_call`'s destructor, typeinfo and vtable from libc++, not weakly (`nm -m -u`). The macOS 26.5 SDK's `c++/v1/__configuration/availability.h` marks that as an LLVM 19 addition, introduced in macOS 15.4. dyld stops at a missing symbol with `Symbol not found`, and adds `(built for macOS 26.0 which is newer than running OS)` only as a note to that error (`Loader::tooNewErrorAddendum` in dyld's `Loader.cpp`). The window built for 13.0 imports none of them.
- **Read: what else a newer build target changes.** clang compiles out every `__builtin_available` check the target already satisfies: v0.9.1's window carries no `__availability_version_check`, and this build's does. The one such check in the window, webview_go's `setInspectable:` (13.3), runs only in debug mode, and the window creates its web view with debug off, so it cost v0.9.1 nothing. AppKit's linked-on-or-after behaviour follows the SDK the main executable was linked against, not its minimum (`_CFExecutableLinkedOnOrAfter`, per [Firefox's primer on macOS SDKs](https://firefox-source-docs.mozilla.org/widget/cocoa/sdks.html)). Both builds link SDK 26.5, so the minimum changes none of that.
- **Decided: 13.0**, for three reasons.
  - The Go 1.27 linker writes `13.0` into every binary it links on its own, the panel and `fleetdeck-status`, and takes no flag for it (`macVersionFlag{13, 0, 0}` in `cmd/link/internal/ld/macho.go`). Go 1.27 runs on nothing older either.
  - A higher minimum would mean linking the panel through clang to change a number, with nothing gained in what it may call.
  - Nothing the window calls needs more. Read from the SDK headers against the window's own C files and webview_go's `webview.h`: the newest API called without asking first is `WKOpenPanelParameters.allowsDirectories`, macOS 10.13.4. The one newer, `WKWebView.inspectable` (13.3), sits behind `__builtin_available`. Such a check only works in a build that names the older macOS: built for 26, clang treats it as always true.
- **Decided: one place, `LSMinimumSystemVersion` in `cmd/fleetdeck-window/Info.plist`.** The Makefile reads it (`MACOS_MIN_VERSION`) and hands it to clang for every binary `window-app` and `dist-app` build (`DARWIN_CGO_ENV`), and so to the release, which builds through `make dist-app`. The plist is what Finder reads, so the promise and the build cannot be moved apart by editing one of them.
- **Measured: cgo flags, not `MACOSX_DEPLOYMENT_TARGET`.** `go build` keys its cache on `CGO_CFLAGS`, `CGO_CXXFLAGS` and `CGO_LDFLAGS`. go 1.27.1's `cmd/go`, `cmd/cgo` and `cmd/link` sources do not mention `MACOSX_DEPLOYMENT_TARGET`, so an object cached by a build without it would be linked in as it was. `CGO_CXXFLAGS` is not optional: with `-mmacosx-version-min=13.0` in `CGO_CFLAGS` and `CGO_LDFLAGS` alone, ld warned `object file (…/000009.o) was built for newer 'macOS' version (26.0) than being linked (13.0)`. webview_go's engine is C++. The binary still named 13.0; the object inside it did not.
- **Decided: the gate demands equal, not "no newer".** `app_runs_on_its_minimum_macos` in `scripts/dist-app-checks.sh`, run by `verify-dist-app` and `verify-dist-dmg`, and so by `make dist-app`, `make dist-dmg` and every verification step of the release workflow: the plist promises exactly the minimum, and every slice of every binary names exactly it. A slice that names an older macOS was built without being told, and is the same defect waiting for a different runner. `TestDistAppBuildsAnAppAPersonCanInstall` reads each slice's `LC_BUILD_VERSION` on its own, and `TestTheAppPromisesTheMacOSItIsBuiltFor` pins the number.
- **Measured: red before the flags, green after.** A `make dist-app` build with the plist already at 13.0 and no flags (`DARWIN_CGO_ENV=` on the command line): `verify-dist-app: fleetdeck-window (arm64) is built for macOS 26.0, not 13.0: what it links and which availability checks it keeps assume that macOS, and Info.plist promises 13.0`, and the Go test failed the same way for both slices. On the installed v0.9.1 the check stops one question earlier: `Info.plist LSMinimumSystemVersion is '11.0', not '13.0'`. After the flags, every slice of all three binaries names `minos 13.0`, with `sdk 26.5` for the window and `26.2` for the two the Go linker links — on this machine and in CI's `oldest-macos-app`, which builds on `macos-latest` as the release does.
- **Not verified: macOS 13 and 14.** GitHub retired its macOS 13 runners in December 2025, and its macOS 14 runners are being retired: jobs fail on purpose on brownout days from 2026-07-06, and the label goes on 2026-11-02. The oldest macOS the app has been opened on is 15.7.9, by CI's `window-on-oldest-macos`. Below that, "it opens" follows from the minimum this build names, the imports read above and the API list; it has not been run. Neither has v0.9.1's `Symbol not found` below 15.4.
