...
- **Measured: v0.9.1 opened on macOS 26 alone.** `xcrun vtool -show-build` on the installed v0.9.1: `fleetdeck-window` named `minos 26.0` in both slices, the panel `minos 13.0`, and `Info.plist` promised `LSMinimumSystemVersion` `11.0`. Nothing in the build named a macOS, so clang built the window for the macOS of the runner it ran on, `macos-latest`, which is macOS 26. dyld refuses a binary that names a newer macOS than the one running it. Finder offered the app to every Mac from 11 on; on anything older than 26 the panel inside would have started and the window would not.
...
- **Measured: red before the flags, green after.** A `make dist-app` build with the plist already at 13.0 and no flags: `verify-dist-app: fleetdeck-window (arm64) is built for macOS 26.0, not 13.0: dyld refuses to load it on any macOS older than that`, and the Go test failed the same way for both slices. On the installed v0.9.1 the check stops one question earlier: `Info.plist LSMinimumSystemVersion is '11.0', not '13.0'`. After the flags, every slice of all three binaries names `minos 13.0`, with `sdk 26.5` for the window and `26.2` for the two the Go linker links.
- **Not verified: macOS 13 and 14.** GitHub retired its macOS 13 runners in December 2025, and its macOS 14 runners are being retired: jobs fail on purpose on brownout days from 2026-07-06, and the label goes on 2026-11-02. The oldest macOS the app has been opened on is 15, by CI's `window-on-oldest-macos`. Below that, "it opens" follows from the minimum dyld reads and the API list above; it has not been run.
