...
- `agent` is the name of an agent *definition*, not a vendor and not a model: it carries the value of Claude Code's `--agent` flag (`Agent for the current session. Overrides the 'agent' setting.`), naming a role from `.claude/agents/` — for this fleet's own sessions, literally `"claude"`. Sessions started without `--agent` have no `agent` key at all, which is the common case. No field on this wire names a model; the model reaches a client only through Claude Code's statusline, never through this socket.
- `needs` is one pre-rendered, human-readable string. When a session is waiting on an actual question it looks like `"answer: <question> (A · B · C)"` or `"choose: <description>"` — the available options, when there are discrete ones, are glued into the text inside parentheses, separated by ` · `. This must be treated as an opaque string and never parsed into structured choices; the parentheses-and-dot format is a rendering choice on the daemon's side, not a contract. `needs` is also non-empty, independent of waiting, for sessions stopped on a reason no answer fixes — a closed vocabulary of prefixes such as `"usage limit reached ..."`, `"rate limited ..."`, `"login required ..."`, `"API error ..."` (see section 5). A non-empty `needs` on its own does **not** mean the session is waiting for a human answer; see the determination rule below. **An absent `needs` and an empty one are different facts and a client must not collapse them.** Present and empty is the daemon answering: it looked, and no one is being asked anything. Absent is a source that never spoke about it — today's daemon always sends the key (measured on 2.1.269: 7 live records of 7, every one carrying `"needs": ""` where there was no question), so absence means a source that does not have this concept, and the determination rule below answers "not known" for it rather than "not waiting".
- `detail` is a short status line, and not always the session's own. On a session waiting via a rendered question in `needs`, it typically holds the same decision text without the glued-in choices. On a session stalled purely by `state`/`tempo` with a `needs` that says nothing — empty or absent (see section 5, rule 4), `detail` is the only field that can still say whether the stop is actually a person-facing decision or just the session coordinating its own subagents — a client must show the session's own words verbatim in that case, not discard them because the session landed in the quiet counter. But the daemon also writes into `detail` the text of the last message sent to the session, and leaves it there until the session says something of its own: measured in the job store's timelines, an `<agent-message …>` envelope opening the text and cut off with an ellipsis, or, when several things queued up while the session was busy, the operator's typed text with an agent's envelope run straight on after it. That text is the sender's, not the session's, and section 5 says what a client does with it.
...
The rule. "Waiting" has three answers, not two — **yes**, **no**, and **not known** — because a source that cannot say has to give some answer, and with only two available that answer is "no": a client would then tell a person, in as many words, that nobody is waiting, on the strength of never having asked. Under-reporting is the failure that is never corrected, so the third answer exists and must be shown as itself.

1. **`needs` absent — not known.** The source said nothing this can be decided from. Not "no": a client must present it as unknown, distinctly from a session it established nobody is waiting on, the same way an absent context estimate is shown as "no transcript yet" rather than as a bar at nought.
2. **`needs` present and non-empty — it decides, and the flags are never read.** A value matching the closed vocabulary below (a usage limit, a login prompt, an upstream API error, a rate limit) means **stalled**, so waiting is **no**. Everything else, including a prefix this client has never seen before, means waiting is **yes**.
3. **`needs` present and empty — waiting is no, unless the session has owed its next move for too long to have said otherwise.** This is an answer rather than a silence: the daemon looked and has no question outstanding. With no words there is still nothing to tell "waiting on a person" apart from "waiting on my own subagents", so rule 4 below never promotes it to yes. The daemon's words come from the session, though, and a session frozen inside a tool call — `ssh` asking about a host key nobody will see, a network call with no timeout, an MCP call that never returns — produces none: measured on a live frozen session against 2.1.269, the daemon went on reporting `needs: ""`, `tempo: "active"` and a `state` that did not change for as long as it stood. So when the session's transcript shows it owing its next move — its newest line is a call that came back or a call still open, or someone addressed it after its last words — and it has said nothing for at least sixteen minutes, this "no" is words the session could not have updated, and waiting is **not known**, exactly as under rule 1. Never yes: nobody established that a person is needed, only that the session could not say. The open call itself is not required and cannot be: Claude Code writes some calls only once they return (a `Read`, alone or in a batch, measured on 2.1.269), so a session frozen inside one shows nothing on disk but its previous result. Sixteen minutes is measured: over fourteen days of transcripts replayed under that write behaviour, working sessions owed their move for at most 651 seconds before their next line appeared, and the worst case the parts add up to — a command at its 601-second ceiling after 338 seconds of the model, p99.99 — is 939 seconds.
4. **`needs` with nothing to say — empty or absent — and `state == "blocked"` or `tempo == "blocked"` — stalled, never waiting.** With no words from the daemon, a bare flag cannot be told apart from a session that is merely coordinating its own subagents; it is never promoted to the counter a person is expected to act on.
5. **Everything else — neither.** The session is making ordinary progress.

"Stalled" stays two-valued, and that is a deliberate asymmetry rather than an oversight. It has the same defect in principle — a source that cannot say reads as "not stalled" — but it lies far more softly, because a session stalled under a source a client cannot read still reaches a person through the silence rule: a session quiet for longer than a threshold calls someone regardless of any flag. That rule is the only one of the three that survives a change of source, because it measures elapsed time rather than trusting the meaning of a field. What it measures is how long ago the session itself last said anything — its own last line in the transcript — and not how long ago the transcript was last written: a message sent to a session lands in its transcript the moment it is sent, so a rule timed by the file would be reset by exactly the person trying to reach a session that cannot answer.

Illustrative examples (not verbatim from any real session):

```text
tempo=blocked  state=working  needs="answer: Which colour should the probe use? (Red · Green · Blue)"   -> waiting: yes        (rule 2)
tempo=active   state=blocked  needs=""                                                                   -> waiting: no, stalled (rules 3, 4)
tempo=blocked  state=blocked  needs="choose: (1) ... (2) ... (3) ..."                                    -> waiting: yes        (rule 2: words outrank both flags)
tempo=active   state=blocked  (no needs key at all)                                                      -> waiting: not known, stalled (rules 1, 4)
```
...
Waiting-yes and stalled are mutually exclusive by construction under this rule: a session with `needs` non-empty lands in exactly one of the two depending on whether it matches the closed vocabulary; a session whose `needs` says nothing — empty or absent — can only be stalled (via the flags) or neither, never waiting-yes. A session can be both not-known and stalled, and there is nothing contradictory in that: the flags say it has stopped, and no one has said whether a person is needed.
...
**A stalled-by-rule-4 session must still show its `detail` verbatim, and so must one whose waiting verdict is not known.** Once `needs` is empty, `detail` is the *only* field left that can distinguish "awaiting a decision from a person" from "awaiting my own work" — a client that hides it because the session landed in the quiet counter — or in no counter at all, which is where a not-known session lands — throws away the one piece of evidence a person could use to notice they are actually needed. This is a presentation obligation on whatever renders the session, not something `Waiting`/`Stalled` themselves can or should encode: the interface that honours it is out of scope for this document.

**The obligation covers the session's own words, and never a message sent to it.** The rule above was written on the assumption that `detail` speaks about the session, and a `detail` holding an incoming message (see section 4) does not: it is somebody else's text, and presenting it as the reason a session stopped tells a person that a sender's words are the session's explanation of itself — an orchestrator writing to a frozen session was seen reading its own letter in that place. Such a `detail` must never be shown as a reason, in a session's row or in any list of reasons. A client may show it, apart from the reason, signed as an incoming message with its sender where the text names one. It is recognised by its envelope: an `<agent-message` tag carrying a `from` that opens the text, or one carrying `id`, `from` and `at`, as the fleet writes them, anywhere in it, where queued messages run together; or a `<cross-session-message` tag carrying a sender. A tag named without those attributes is text — a session's status may well name the tag it is waiting for. A prompt typed or queued into a session with no envelope at all cannot be told from the session's own words by this field alone, so this obligation cannot be kept for it from `detail`.
