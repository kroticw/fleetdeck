...
3. **`needs` present and empty — waiting is no, unless the session has been silent inside one tool call for too long to have said otherwise.** This is an answer rather than a silence: the daemon looked and has no question outstanding. With no words there is still nothing to tell "waiting on a person" apart from "waiting on my own subagents", so rule 4 below never promotes it to yes. The daemon's words come from the session, though, and a session frozen inside a tool call — `ssh` asking about a host key nobody will see, a network call with no timeout, an MCP call that never returns — produces none: measured on a live frozen session against 2.1.269, the daemon went on reporting `needs: ""`, `state: "working"`, `tempo: "active"` for as long as it stood. So when the session's transcript shows it inside one tool call and it has said nothing for at least eleven minutes, this "no" is words the session could not have updated, and waiting is **not known**, exactly as under rule 1. Never yes: nobody established that a person is needed, only that the session could not say. Eleven minutes is measured: over fourteen days of transcripts, no tool call that returns stayed silent for longer than 601 seconds, which is where Claude Code moves a command to the background when its timeout runs out.
