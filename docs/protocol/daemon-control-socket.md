...
3. **`needs` present and empty — waiting is no.** This is an answer rather than a silence: the daemon looked and has no question outstanding. With no words there is still nothing to tell "waiting on a person" apart from "waiting on my own subagents", so rule 4 below never promotes it to yes.
