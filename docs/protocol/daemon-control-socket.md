...
The marker is written flush against whatever PTY bytes were already in flight — it is **not** anchored to the start of a line or to offset 0 of the stream. A terminal screen almost never ends with a trailing newline (the cursor sits wherever it sits: mid-escape-sequence, mid-prompt, mid-line), so a detector that only accepts the marker at offset 0 or immediately after a `\n` will miss it in the common case. The one invariant that does hold is that the marker and its reason text are the *last* bytes sent before the connection closes; a client should locate the marker relative to the end of the accumulated stream (and require the connection to have actually closed right after) rather than relative to a line boundary. The same literal text can legitimately appear earlier in an ordinary screen as unrelated content — this very document contains the string `EKICKED:` several times — so detection must never fire on the marker's presence alone, only on it being the last thing sent immediately before a close.
