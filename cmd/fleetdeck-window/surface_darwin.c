...
static const char *nameOf(id self) { return cstring(objc_getAssociatedObject(self, &nameKey)); }

static void setName(id object, const char *name) {
  objc_setAssociatedObject(object, &nameKey, nsstring(name), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
